...
def plot_points(points, selected_idx=None, color='#27AE60', figsize=(6, 6),
                 arrow_diameter=ARROW_DIAMETER_CM, show_score_label=True, show_centroid=True):
    """
    ...
    show_centroid: 重心位置を×マーカーで表示するか
    """
    ...
    if len(points) > 0:
        ...
        if show_centroid:
            ...
            ax.plot(centroid[0], centroid[1], marker='x', markersize=16,
                   markeredgewidth=3, color='#0074D9', zorder=6, linestyle='None')
            ax.annotate("重心", (centroid[0], centroid[1]), textcoords="offset points",
                       xytext=(10, -12), fontsize=10, color='#0074D9', fontweight='bold')
